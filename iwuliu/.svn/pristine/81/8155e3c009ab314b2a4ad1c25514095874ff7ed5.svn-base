    ...
    public void scheduleFlowTable(HttpServletRequest request,HttpServletResponse response,String schedulingSheetId){
        ...
        try {
            ...
            for (T_Data_Transportation_Schedule_Finance_Record record : recordList) {
                ...
                Date parse2 = format.parse(createTime2);
                map.put("createTime2",format.format(parse2));
               ...
               // map.put("latitude",latitude);
                String responses = null;
                Map<String, Object> retList = null;
                String addressStr = "http://restapi.amap.com/v3/geocode/regeo?output=JSON&location=" + latitude + ',' + longitude + "&key=5107d8c6eaacd4c780ed2a820e3680fc&radius=1000&extensions=all&roadlevel=1&batch=false";
                Gson gson = new Gson();
                HttpClient client = new HttpClient();
                HttpMethod method = new GetMethod(addressStr);
                client.executeMethod(method);
                ...
                String position="";
                Object status = retList.get("status");
                if (!status.equals("0")){
                    Map<String, Object> ret = (Map<String, Object>) retList.get("regeocode");
                    position = (String) ret.get("formatted_address");
                }
