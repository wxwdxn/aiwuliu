/**
 * 版权所有：版权所有(C) 2015
 * 文件名称：SettleConfirmApiController.java
 * 系统编号: Z0001002
 * 系统名称：物流管理平台
 * 模块编号：
 * 模块名称：
...
/**
 * 类 名 称：SettleConfirmApiController
 * 内容描述：
 * 方法描述：该类有 个方法
 *          01 
 *@authot YK
...
public class SettleConfirmApiController {
    Logger logger = Logger.getLogger(SettleConfirmApiController.class);

    public String formatStr(String text) {
        return text == null ? "" : text.trim();
    }

    @RequestMapping(value = "submit_settleConfirm")
    protected void submit_settleConfirm(HttpServletRequest request, HttpServletResponse response,ModelMap model){
        ...
        try {
            ...
            RequestDispatcher view	= request.getRequestDispatcher("/yeePayManager/home_settleConfirmResp");
