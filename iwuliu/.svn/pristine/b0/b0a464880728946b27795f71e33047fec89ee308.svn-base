/**
 * 版权所有：版权所有(C) 2015
 * 文件名称：SubLineNodeService.java
 * 系统编号: Z0001002
 * 系统名称：物流管理平台
 * 模块编号：02
 * 模块名称：干线路线节点基础息查询接口声明
 * 设计文件：
 * 完成日期：2016-06-22
 * 作    者: QJ
 * 内容摘要：干线路线节点基础息查询
 ...
 */
@WebService
public interface SubLineNodeService {
    // 根据ID查询干线路线节点基础信息
    T_Master_Sub_Line_Detail findSubLineNodeById(String line_id);

    // 根据干线路线ID查询干线路线节点基础信息
    List<T_Master_Sub_Line_Detail> findSubLineNodeByInfoID(String main_line_info_id);

    // 根据节点编号查询干线路线节点基础信息
    T_Master_Sub_Line_Detail findSubLineNodeByNodeNo(String node_no);

    // 根据节点名称查询干线路线节点基础信息
    T_Master_Sub_Line_Detail findSubLineNodeByNodeName(String node_name);

    // 查询符合条件的干线路线节点列表信息（默认查询干线路线节点列表信息）
    List<T_Master_Sub_Line_Detail> findAllSubLineNode(String search_type, String name, Integer page, Integer rows);
    ...
    // 更新干线路线节点基础信息
    boolean updateSubLineNode(T_Master_Sub_Line_Detail sub_line_node);

    // 根据ID删除干线路线节点基础信息
    void delSubLineNode(String sub_line_node);
}