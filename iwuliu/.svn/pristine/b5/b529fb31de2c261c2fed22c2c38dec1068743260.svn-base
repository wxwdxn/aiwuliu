 ...
 * @author WXW
 */
public class T_Data_Transportation_OrderList {

    private String schedule_plan_number;//订单ID
    ...
    //根据距离倒序
   public List<T_Data_Transportation_OrderList> mySort(List<T_Data_Transportation_OrderList> arr){
       T_Data_Transportation_OrderList sort= null;
       for (int i=0;i<arr.size();i++){
           for (int j=arr.size()-2;j>=i;j--){
               double juli = Double.parseDouble(arr.get(j).getJuli());
               double juli2 = Double.parseDouble(arr.get(j+1).getJuli());
               if (juli>=juli2){
                   sort = arr.get(j+1);
