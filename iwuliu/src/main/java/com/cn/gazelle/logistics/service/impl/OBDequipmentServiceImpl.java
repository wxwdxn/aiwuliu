...
public class OBDequipmentServiceImpl implements OBDequipmentService {
    ...
    public int saveEquipmentInfo(String info, String userName) {
        ...
        int flag = 0;
        truck = this.truckDaoMapper.findTruckByPlateNumber(data.get("add_plate_number"));
        // 判断输入的车牌号是否存在
        if (truck != null) {
            // 判断该车牌号是否绑定过obd设备
            if (this.obdEquipmentTruckBindingDaoMapper.findOBDTruckBindingInfoByTruckID(truck.getTruck_id()) != null) {
                ...
                try {
                    // 判断obd_id是否绑定车牌
                    T_Data_OBD_Equipment equipment = this.obDequipmentDaoMapper.findByObdId(data.get("add_obd_id"));
                    if (equipment!=null){
                    ...
                    obdEquipment.setBrand_id(data.get("add_obd_brand"));
                    obdEquipment.setObd_id(data.get("add_obd_id"));
                    obdEquipment.setEquipment_phone_number(data.get("add_equipment_phone_number"));
                    obdEquipment.setRegister_time(sdf.parse(data.get("add_register_time")));
        ...
        try {
            ...
            if (obdEquipmentTruckBinding != null) {
                ...
                if (obdEquipment != null) {
                    if (this.obDequipmentDaoMapper.findByObdId(data.get("detail_obd_id")) != null) {
                        // 判断设备id是否未作修改
                        if (data.get("detail_obd_id").equals(obdEquipment.getObd_id())){
                            obdEquipment.setBrand_id(data.get("detail_obd_brand"));
                            obdEquipment.setObd_id(data.get("detail_obd_id"));
                            obdEquipment.setEquipment_phone_number(data.get("detail_equipment_phone_number"));
                            obdEquipment.setRegister_time(sdf.parse(data.get("detail_register_time")));
                            obdEquipment.setLast_update(DateUtil.getDate());
                            obdEquipment.setLast_update_user_id("U:" + username);
