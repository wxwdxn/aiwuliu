/**
 * 版权所有：版权所有(C) 2015
 * 文件名称：CashTransferApiController.java
 * 系统编号: Z0001002
 * 系统名称：物流管理平台
 * 模块编号：
 * 模块名称：
...
/**
 * 类 名 称：CashTransferApiController
 * 内容描述：
 * 方法描述：该类有 个方法
 *          01 
 *@authot YK
...
public class CashTransferApiController {
    Logger logger = Logger.getLogger(CashTransferApiController.class);

    public String formatStr(String text) {
        return text == null ? "" : text.trim();
    }

    @RequestMapping(value = "submit_cashTransfer")
    protected void submit_cashTransfer(HttpServletRequest request, HttpServletResponse response,ModelMap model)  {
...
//            }
            ...
            RequestDispatcher view	= request.getRequestDispatcher("/yeePayManager/home_cashTransferResp");
