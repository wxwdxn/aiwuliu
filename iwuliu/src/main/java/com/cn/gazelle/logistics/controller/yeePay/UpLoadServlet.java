...
public class UpLoadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			File fileUp = new File(sc.getRealPath("/") + "path");
			if (!fileUp.exists()){
				fileUp.mkdirs();
			}
			while (itr.hasNext()) {
		...
		try {
			...
			RequestDispatcher view	= request.getRequestDispatcher("/yeePayManager/home_uploadResp");
	...
	public static void main(String[] args) {
		...
		for (String p : person) {
			...
			dataMap.put("filetype", filetype);
			dataMap.put("hmac", hmac); // hmac 按照 properties 中声明的顺序进行签名
			String dataJsonString = JSON.toJSONString(dataMap); // map
																						// 中数据转为
																						// json
																						// 格式
			String content = AESUtil.encrypt(dataJsonString, key);
