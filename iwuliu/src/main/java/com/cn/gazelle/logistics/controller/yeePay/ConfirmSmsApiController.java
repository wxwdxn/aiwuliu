/**
 * 版权所有：版权所有(C) 2015
 * 文件名称：ConfirmSmsApiController.java
 * 系统编号: Z0001002
 * 系统名称：物流管理平台
 * 模块编号：
 * 模块名称：
 ...
 */
@Controller
@RequestMapping(value = "/confirmSms")
public class ConfirmSmsApiController {
    Logger logger = Logger.getLogger(ConfirmSmsApiController.class);

    public String formatStr(String text) {
        return text == null ? "" : text.trim();
    }

    @RequestMapping(value = "submit_confirmSms")
    protected void submit_confirmSms(HttpServletRequest request, HttpServletResponse response,ModelMap model)  {
        ...
        try {
            ...
            RequestDispatcher view	= request.getRequestDispatcher("/yeePayManager/home_confirmSmsResp");
